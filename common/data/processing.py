...
def clip_image_preprocessor(images = None, img_size=224, resampling = 3):
    """
    Image procesor using CLIP means/stds that can be used directly as callable or to create
...
@torch.no_grad()
def clip_image_postprocessor(model_out, to_pil = False):
    for i in range(3):
...
def common_image_postprocessor(model_out, to_pil = False):
    for i in range(3):
        model_out[:,i] = model_out[:,i] * 0.5 + 0.5
    model_out = model_out.clamp(0, 1)
    model_out = model_out.permute(0, 2, 3, 1)
    model_out = (model_out.float() * 255).detach().cpu().numpy().astype(np.uint8)
    return [Image.fromarray(img) if to_pil else img for img in model_out]
