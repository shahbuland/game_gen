...
class LADD(MixIn):
    ...
    def teacher_discriminate(self, real, fake, text_features, timesteps):
        _, h_real = self.teacher.predict(real, text_features, timesteps, output_hidden_states = True)
        _, h_fake = self.teacher.predict(fake, text_features, timesteps, output_hidden_states = True)
    ...
    def forward(self, text_features : TensorType["b", "n", "d"]):
        ...
        samples, noise = self.teacher_sample(text_features).detach()
        ...
        student_v_pred = self.student.predict(perturbed, text_features, timesteps_distill) # (x - z)
        ...
        perturbed_adv_real = rectflow_lerp(samples, noise, timesteps_distill)

        if self.train_disc:
            loss, _ = self.teacher_discriminate(perturbed_adv_real, perturbed_adv_fake, text_features, timesteps_adv)
            return loss
        else:
            ...
            _, adv_loss = self.teacher_discriminate(perturbed_adv_real, perturbed_adv_fake, text_features, timesteps_adv)
...
if __name__ == "__main__":
    ...
    text_encoder = CLIPConditioner(
        core_model = CLIPTextModel.from_pretrained("openai/clip-vit-base-patch32"),
        layer_skip = -1,
        hidden_size = 768
    )

    model = LADD(
        ConditionedRectFlowTransformer(model_config, text_encoder),
        ConditionedRectFlowTransformer(model_config, text_encoder),
        model_config, disc_config
    ) 
    model.cuda()

    test_input = torch.randn(4, 77, 768, device = 'cuda')
